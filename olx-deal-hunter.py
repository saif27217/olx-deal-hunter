...
def main():
    ...
    # Filter out suspicious deals entirely (score < 20 = likely scam)
    filtered = [d for d in unique if d["score"] >= 20]
    
    # Sort: recency DESC (newest first), then composite score DESC, then price ASC
    all_deals = sorted(filtered, key=lambda d: (-d["days_ago"], -d["score"], d["price"]))
    save_seen(seen)
    ...
    try:
        ...
        # Summary header
        # Top 3: only legitimate deals (legit_score >= 80 = fair/below market)
        top3 = [d for d in all_deals if d["legit_score"] >= 80][:3]
        # Fallback: if fewer than 3 legit deals, show best scores
        if len(top3) < 3:
            top3 = all_deals[:3]
        top_items = []
        for d in top3:
...
|---|-------|--------|---------|---------|-----|-------|------------|---------|---------|-------|-------|-------------|------|"""
        ...
        for i, d in enumerate(all_deals[:50], 1):
            ...
            sc = d["score"]
            star = stars(sc)
            if sc >= 70: score_badge = f"{star} {sc}"
            elif sc >= 50: score_badge = f"{star} {sc}"
            elif sc >= 20: score_badge = f"{star} {sc}"
...
- **Sorted by:** Recency (newest first) → Score (highest first)
